...
### 线程
创建线程之后，地址空间没有变化，进程随即退出成了线程[主线程]
...
如果没有设置分离，那么需要父线程去回收子线程的pcb[默认是不分离的]
...
3、条件变量[本质不是锁]，用来阻塞线程，需要搭配互斥锁一起使用[比如实现生产者、消费者模型]
注意：
    互斥锁主要是用来保护共享资源，条件变量用来阻塞线程，条件变量一般需要搭配互斥锁一起使用
