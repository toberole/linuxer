...
dup
  int dup(int oldfd);
        dup操作返回一个指向oldfd所指向的文件的newfd,当关闭oldfd时候，文件并不会关闭，可以继续使用返回的newfd
        oldfd、newfd任意一个文件描述符发生读写，都对应同一个文件的底层的数据结构
...
阻塞信号集：要屏蔽的信号
未决信号集：没有被处理的信号的集合[阻塞信号 也是没有被处理的信号]
...
2、配置 在/etc/xinetd.d目录下面，创建一个services的配置，直接拷贝里面的services ，修改即可
3、修改/etc/services文件 设置端口监听

### 判断文件是否存在
1 access
2 stat[判断未打开的文件]、fstat[判断打开的文件]
3 open
注意： 通过返回值判断
...
### 设置缓冲 不是用系统默认的缓冲
setbuf、setvbuf
使用 setbuf 与 setvbuf 函数指定文件的缓冲区一定要在文件读写之前。
一旦用户自己指定了文件的缓冲区，文件的读写就要在用户指定的缓冲区中进行，而不在系统默认指定的缓冲区中进行。
...
<pre>
    C语言中static变量属于静态存储方式，位于内存中的静态存储去，该区域中的数据以程序整个生命周期内都存在,一直占据着内存。

    属于静态存储方式的量不一定就是静态变量，比如外部变量属于静态存储方式，但不一定是静态变量。

    函数体内如果在定义静态变量的同时进行了初始化，则以后程序不再进行初始化操作.

    静态局部变量的初始化表达式必须是一个常量或者常量表达式。即使局部静态变量定义时没有赋初值，系统会自动赋初值0.
...
2.作用域，全局的静态变量、静态函数只能在定义它的文件中使用。
